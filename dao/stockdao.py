...
class AlmacenStockBD:
    ...
    def buscar_almacen_stock(self, codalmacenstock):
        with self.conexion.cursor() as cursor:
            sql = """SELECT fecha_ingreso, cantidad_producto, nombre_producto, cod_empleado, cod_producto
                     FROM almacen_stock WHERE cod_almacen_stock = '{}'""".format(codalmacenstock)
            cursor.execute(sql)
    ...
    def insertar_almacen_stock(self, almacenstock):
        with self.conexion.cursor() as cursor:
            ...
            WHERE cod_producto = '{}'
            """.format(almacenstock.cantidad_producto, almacenstock.cod_producto)
            ...
            cursor.execute(sql_insert_almacen_stock)
